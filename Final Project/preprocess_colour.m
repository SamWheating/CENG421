function clean = preprocess_colour(filename)
% takes a filename of a leaf image, returns binary mask of leaf (with stem removed?)
image = imread(filename);

image2 = imcomplement(image(:,:,2));

% imshow(image)

BW = imbinarize(image2, 0.10);
...
image = imrotate(image, 90-leaf_props(1).Orientation);
image = image .* uint8(BW);

% imshow(BW)

clean = image;        % returns a veritcally-oriented binary image
